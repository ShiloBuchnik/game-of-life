...
int main() {
    ...
    int mode_num = 2;
    while (mode_num <= 28 ) {
        Patterns::putPatternInGrid(grid, *Patterns::numToPattern[mode_num]);
        while (window.isOpen()) {
            sf::Event evnt;
            while (window.pollEvent(evnt)) {
                switch (evnt.type) {
                    ...
                    case sf::Event::KeyPressed:
                        ...
                        if (evnt.key.code == sf::Keyboard::W || evnt.key.code == sf::Keyboard::A || evnt.key.code == sf::Keyboard::S ||
                            evnt.key.code == sf::Keyboard::D) {
